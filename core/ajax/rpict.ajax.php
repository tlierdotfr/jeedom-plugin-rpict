...
try {
    ...
    switch (init('action')){
        ...
        break;
        case 'restartDeamon':
            $port                 = config::byKey('port', 'rpict', 'none');
            if ($port == 'none') {
                ajax::success();
            }
            rpict::deamon_stop();
        ...
        case 'getRpict':
            ...
            foreach ($object->getEqLogic() as $eqLogic) {
                if ($eqLogic->getIsVisible() == '1' && $eqLogic->getEqType_name() == 'rpict') {
        ...
        case 'getHealth':
            if (init('eqLogicID') !== null) {
                $rpict       = rpict::byLogicalId(init('eqLogicID'), 'rpict');
                $health_cmd     = $rpict->getCmd('info', 'health');
                $return         = array('object' => utils::o2a($health_cmd));
                $return["NodeID"] = init('eqLogicID');
                ajax::success($return);
            } else {
                $rpict = rpict::byType('rpict');
                foreach ($rpict as $eqLogic) {
                    $health_cmd     = $eqLogic->getCmd('info', 'health');
                    $return         = array('object' => utils::o2a($health_cmd));
                    $return["NodeID"] = $eqLogic->getLogicalId();
                    ajax::success($return);
                }
            }
        ...
        case 'getInfoDaemon':
            ...
            $path = realpath(dirname(__FILE__) . '/../../ressources/rpict.log');
            if (!file_exists($path)) {
