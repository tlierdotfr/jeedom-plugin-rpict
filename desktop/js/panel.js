...
$('#bt_rpictPanelSante').on('click', function() {
    $('#md_modal').dialog({title: "{{Santé}}"});
    $('#md_modal').load('index.php?v=d&plugin=rpict&modal=panel_sante').dialog('open');
});
...
$.ajax({
                        ...
                        try{
                            switch(data.result[eqLogic].cmd[cmd].logicalId)
                            {
                                ...
                                case "STAT_TODAY":
                                    ...
                                    $('.rpictAttr[data-l1key=conso][data-l2key=day]').text((data.result[eqLogic].cmd[cmd].value)/1000);
                                    break;
                                case "STAT_MONTH":
                                    console.log("[loadData][STAT_MONTH] " + data.result[eqLogic].cmd[cmd].value);
                                    $('.rpictAttr[data-l1key=conso][data-l2key=month]').text((data.result[eqLogic].cmd[cmd].value)/1000);
                                    break;
                                case "STAT_MONTH_LAST_YEAR":
                                    console.log("[loadData][STAT_MONTH_LAST_YEAR] " + data.result[eqLogic].cmd[cmd].value);
                                    $('.rpictAttr[data-l1key=conso][data-l2key=monthlastyear]').text((data.result[eqLogic].cmd[cmd].value)/1000);
                                    break;
                                case "STAT_YEAR_LAST_YEAR":
                                    console.log("[loadData][STAT_YEAR_LAST_YEAR] " + data.result[eqLogic].cmd[cmd].value);
                                    $('.rpictAttr[data-l1key=conso][data-l2key=yearlastyear]').text((data.result[eqLogic].cmd[cmd].value)/1000);
                                    break;
                                case "STAT_YEAR":
                                    console.log("[loadData][STAT_YEAR] " + data.result[eqLogic].cmd[cmd].value);
                                    //console.log(data.result[eqLogic].cmd[cmd]);
                                    $('.rpictAttr[data-l1key=conso][data-l2key=year]').text((data.result[eqLogic].cmd[cmd].value)/1000);
                                    break;
                                case "STAT_JAN_HP":
    ...
    /*var pourcentMonth = ($('.rpictAttr[data-l1key=conso][data-l2key=monthlastyear]').text() / $('.rpictAttr[data-l1key=conso][data-l2key=month]').text()) *100;
...
function getRpictObjectHistory(div, type, object) {
    $.ajax({
        type: 'POST',
        async:true,
        url: 'plugins/rpict/core/ajax/rpict.ajax.php',
